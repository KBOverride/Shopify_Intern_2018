...
for (var i = 1; i <= 5; i++) {
	apiPromises.push(fetch('https://backend-challenge-summer-2018.herokuapp.com/challenges.json?id=2&page=' + i).then((resp) => resp.json()));
}
...
function settingNodes(obj) {
	for(var pg_count = 0; pg_count < 5; pg_count++) {
